...
function pickPagination(payload){
  ...
  const totalPages = Number(meta.last_page || meta.total_pages || meta.pages || payload?.total_pages || 0) || 0;
  const totalItems = Number(meta.total || payload?.total || payload?.count || 0) || 0;
...
export default async function handler(req, res) {
  try {
    ...
    while (page <= MAX_PAGES) {
      ...
      if (reachedLastKnownPage || receivedShortPage || reachedKnownTotal) {
        break;
      }
