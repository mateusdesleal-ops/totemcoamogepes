...
<div class="presentationAuthor__text">
  ...
  function firstValidUrl(values){
    ...
    for(const value of list){
      ...
      if(!clean) continue;
      if(/^https?:\/\//i.test(clean) || /^assets\//i.test(clean) || /^\.\//.test(clean) || /^\//.test(clean)) return clean;
    }
    return "";
  }

  function findRemoteImage(job){
    ...
    const nestedObjects = [job.media, job.images, job.image_data, job.assets, job.files, job.company, job.vacancy, job.job];
    for(const obj of nestedObjects){
      if(!obj || typeof obj !== "object") continue;
      const nested = firstValidUrl([
        obj.url, obj.src, obj.href, obj.image, obj.image_url, obj.thumbnail, obj.thumbnail_url,
        obj.cover, obj.cover_url, obj.banner, obj.banner_url, obj.photo, obj.photo_url
      ]);
      if(nested) return nested;
    }

    for(const [key, value] of Object.entries(job)){
      if(typeof value === "string" && /(image|thumb|photo|cover|banner|hero|background)/i.test(key)){
        const url = firstValidUrl(value);
        if(url) return url;
      }
      if(value && typeof value === "object" && /(image|thumb|photo|cover|banner|hero|background)/i.test(key)){
        const url = firstValidUrl([value.url, value.src, value.href, value.image, value.image_url, value.thumbnail, value.cover]);
        if(url) return url;
      }
    }
  ...
  function getVacancyImage(v){
    const remote = firstValidUrl([v?.image, v?.raw?.image, findRemoteImage(v?.raw)]);
  ...
  function guessGroup(job){
    const raw = String(job.group || job.area || job.department || job.category || job.segment || job.business_unit || "admin").toLowerCase();
  ...
  function normalizeVacancy(job, idx){
    const location = job.location || job.city || job.unit || job.workplace || [job.address_city, job.address_state].filter(Boolean).join(", ") || "";
    return {
  ...
  async function fetchVacanciesFromApi(){
    ...
    const url = new URL(jf.endpoint, `${base}/`);
    ...
    arr.forEach((x, idx) => {
      const item = normalizeVacancy(x, idx);
